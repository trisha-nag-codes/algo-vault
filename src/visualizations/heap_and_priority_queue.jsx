...
const PROBLEMS = {
  kthLargest: {
    title: "Kth Largest",
    subtitle: "LC 215 Â· Min-Heap of Size K",
    coreIdea: "Maintain a min-heap of size k. For each element: if heap has room, push. If the element beats the current min (heap[0]), replace it. After processing all elements, heap[0] is the kth largest â€” everything else in the heap is larger. O(n log k) beats O(n log n) sorting when k â‰ª n.",
    heapType: "min",
  },
  mergeKLists: {
    title: "Merge K Lists",
    subtitle: "LC 23 Â· Min-Heap of K Pointers",
    coreIdea: "Seed a min-heap with the head of each sorted list â€” the heap always holds exactly one element per active list. Pop the global minimum, append to result, advance that list's pointer and push its next element. The heap keeps extraction O(log k), giving O(N log k) total where N is the count across all lists.",
    heapType: "min",
  },
  kClosest: {
    title: "K Closest Pts",
    subtitle: "LC 973 Â· Max-Heap of Size K",
  ...
  taskScheduler: {
    ...
    subtitle: "LC 621 Â· Max-Heap + Cooldown",
...
function buildKthSteps() {
  ...
  const snap = () => [...heap];
  steps.push({ title: "Initialize â€” Empty Min-Heap", detail: `nums = [${arr.join(", ")}], k = ${k}. Maintain min-heap of size â‰¤ ${k}. After all elements, heap[0] = kth largest.`, heap: snap(), phase: "init", codeHL: [2,3], highlighted: [], extra: { inputIdx: -1, log: [], expected } });
  for (let i = 0; i < n; i++) {
    const num = arr[i];
    if (heap.length < k) {
      minPush(heap, num); log.push(`push ${num}`);
      steps.push({ title: `Push ${num} â€” Size ${heap.length} â‰¤ k=${k}`, detail: `Heap has room. Push ${num}, sift up. heap = [${heap.join(", ")}].`, heap: snap(), phase: "push", codeHL: [4,5,6], highlighted: [heap.indexOf(num)], extra: { inputIdx: i, log: [...log], expected } });
    } else if (num > heap[0]) {
      ...
      log.push(`${old}â†’${num}`);
      steps.push({ title: `Replace ${old} â†’ ${num} â€” ${num} > heap[0]=${old}`, detail: `${num} belongs in top-${k}. Evict ${old}, sift down. heap = [${heap.join(", ")}].`, heap: snap(), phase: "replace", codeHL: [7,8], highlighted: [0], extra: { inputIdx: i, log: [...log], expected } });
    } else {
      log.push(`skip ${num}`);
      steps.push({ title: `Skip ${num} â€” ${num} â‰¤ heap[0]=${heap[0]}`, detail: `Can't be in top ${k}. heap = [${heap.join(", ")}].`, heap: snap(), phase: "skip", codeHL: [7], highlighted: [], extra: { inputIdx: i, log: [...log], expected } });
    }
  }
  steps.push({ title: `âœ“ Answer: heap[0] = ${heap[0]}`, detail: `Min-heap of size ${k}: [${heap.join(", ")}]. kth largest = ${heap[0]}.`, heap: snap(), phase: "done", codeHL: [9], highlighted: [0], extra: { inputIdx: n, log: [...log], expected, result: heap[0] } });
  return steps;
}
...
function buildMergeSteps() {
  ...
  steps.push({ title: `Initialize â€” Seed Heap with ${k} List Heads`, detail: `${k} sorted lists. Push head of each into min-heap. Heap always holds one element per active list.`, heap: snapH(), phase: "init", codeHL: [2,3,4,5], highlighted: [], extra: { merged: snapM(), ptrs: [...ptrs], expected } });
  for (let li = 0; li < k; li++) tMinPush(heap, [lists[li][0], li, 0]);
  steps.push({ title: `Heap Seeded â€” [${heap.map(h=>h[0]).join(", ")}]`, detail: `Heads: ${lists.map((l,i)=>`L${i}[0]=${l[0]}`).join(", ")}. Min = ${heap[0][0]}.`, heap: snapH(), phase: "push", codeHL: [4,5], highlighted: Array.from({length:k},(_,i)=>i), extra: { merged: snapM(), ptrs: [...ptrs], expected } });
  while (heap.length > 0) {
    ...
    steps.push({ title: hasN ? `Pop ${val} (L${li}) â†’ Push ${lists[li][nxt]}` : `Pop ${val} (L${li}) â€” List Exhausted`, detail: `Pop min=${val} from L${li}[${ei}], append to result.${hasN ? ` Push L${li}[${nxt}]=${lists[li][nxt]}.`:` L${li} done.`} merged=[${merged.join(",")}].`, heap: snapH(), phase: hasN ? "pop-push" : "pop-done", codeHL: hasN ? [8,9,10,11,12] : [8,9,10], highlighted: hasN ? [heap.findIndex(h=>h[1]===li)] : [], extra: { merged: snapM(), ptrs: [...ptrs], expected } });
  }
  steps.push({ title: `âœ“ All ${merged.length} Elements Merged`, detail: `Result: [${merged.join(", ")}]. O(N log k), N=${merged.length}, k=${k}.`, heap: snapH(), phase: "done", codeHL: [13], highlighted: [], extra: { merged: snapM(), ptrs: [...ptrs], expected, result: merged } });
  return steps;
}
...
function buildClosestSteps() {
  ...
  steps.push({ title: "Initialize â€” Empty Max-Heap by Distance", detail: `${n} points, k=${k}. Max-heap of size â‰¤ ${k} by dÂ². Farthest at top â€” easy to evict.`, heap: snapH(), phase: "init", codeHL: [2,3], highlighted: [], extra: { inputIdx: -1, log: [] } });
  for (let i = 0; i < n; i++) {
    const d = points[i][0]**2 + points[i][1]**2;
    if (heap.length < k) {
      tMaxPush(heap, [d, i]); log.push(`push ${fmtPt(i)}`);
      steps.push({ title: `Push ${fmtPt(i)} dÂ²=${d} â€” Size ${heap.length}â‰¤k`, detail: `Room in heap. Push. Top dÂ²=${heap[0][0]}, size=${heap.length}.`, heap: snapH(), phase: "push", codeHL: [5,6,7], highlighted: [heap.findIndex(h=>h[1]===i)], extra: { inputIdx: i, log: [...log] } });
    } else if (d < heap[0][0]) {
      const evi = heap[0][1]; tMaxPop(heap); tMaxPush(heap, [d, i]); log.push(`${fmtPt(evi)}â†’${fmtPt(i)}`);
      steps.push({ title: `Replace ${fmtPt(evi)} â†’ ${fmtPt(i)}  dÂ²=${d}<${DISTS[evi]}`, detail: `${fmtPt(i)} closer. Evict ${fmtPt(evi)}. Top dÂ²=${heap[0][0]}.`, heap: snapH(), phase: "replace", codeHL: [8,9], highlighted: [0], extra: { inputIdx: i, log: [...log] } });
    } else {
      log.push(`skip ${fmtPt(i)}`);
      steps.push({ title: `Skip ${fmtPt(i)} dÂ²=${d} â‰¥ ${heap[0][0]}`, detail: `Not closer than farthest candidate. Skip.`, heap: snapH(), phase: "skip", codeHL: [8], highlighted: [], extra: { inputIdx: i, log: [...log] } });
    }
  }
  steps.push({ title: `âœ“ ${k} Closest Points Found`, detail: `Heap: ${heap.map(h=>`${fmtPt(h[1])}(dÂ²=${h[0]})`).join(", ")}. O(n log k).`, heap: snapH(), phase: "done", codeHL: [10], highlighted: Array.from({length:heap.length},(_,i)=>i), extra: { inputIdx: n, log: [...log], result: heap.map(h=>points[h[1]]) } });
  return steps;
}
...
function buildMedianSteps() {
  ...
  steps.push({ title: "Initialize â€” Two Empty Heaps", detail: `Stream: [${stream.join(", ")}]. maxHeap (lower half), minHeap (upper half). Keep |maxHeap| â‰¥ |minHeap|, differ by â‰¤ 1.`, heap: [], phase: "init", codeHL: [2,3,4,5], highlighted: [], extra: { lo: snapLo(), hi: snapHi(), medians: [], stream, expected } });
  for (let i = 0; i < stream.length; i++) {
    ...
    steps.push({ title: `Add ${num} â†’ Median = ${fmt(med)}`, detail: `Insert ${num}.${rebal ? " Rebalanced." : ""} maxHeap=[${lo.join(",")}] top=${lo[0]}, minHeap=[${hi.length?hi.join(","):"âˆ…"}]${hi.length?" top="+hi[0]:""}. ${total%2===1?`Odd â†’ median = ${med}`:`Even â†’ (${lo[0]}+${hi[0]})/2 = ${fmt(med)}`}.`, heap: [], phase: "insert", codeHL: [7,8,9,10,11], highlighted: [], extra: { lo: snapLo(), hi: snapHi(), medians: [...medians], stream, expected, currentIdx: i } });
  }
  steps.push({ title: `âœ“ All ${stream.length} Medians Computed`, detail: `Medians: [${medians.map(m=>m%1===0?m:m.toFixed(1)).join(", ")}]. Two-heap O(log n) per insert.`, heap: [], phase: "done", codeHL: [13,14,15,16], highlighted: [], extra: { lo: snapLo(), hi: snapHi(), medians: [...medians], stream, expected, result: medians } });
  return steps;
}
...
function buildSchedulerSteps() {
  ...
  steps.push({ title: "Initialize â€” Frequencies into Max-Heap", detail: `Tasks: [${tasks.join(",")}], cooldown n=${cd}. Freq: ${Object.entries(freq).map(([t,c])=>`${t}:${c}`).join(", ")}. Heap: [${heap.map(h=>`${h[1]}:${h[0]}`).join(", ")}].`, heap: snapH(), phase: "init", codeHL: [3,4,5,6,7,8], highlighted: [], extra: { timeline: [], cooldownQ: snapQ(), expected } });
  let time = 0;
  while (heap.length > 0 || coolQ.length > 0) {
    ...
    if (heap.length > 0) {
      ...
      steps.push({ title: `t=${time}: Execute ${task} (${cnt}â†’${cnt-1})`, detail: `${returned.length?`Returned: [${returned.join(",")}]. `:""}Pop ${task} (freq=${cnt}). ${cnt-1>0?`Cooldown until t=${time+cd+1}.`:`${task} done.`} Timeline: [${timeline.join(",")}].`, heap: snapH(), phase: "execute", codeHL: [9,10,13,14,15], highlighted: [], extra: { timeline: [...timeline], cooldownQ: snapQ(), expected, returned: [...returned] } });
    } else {
      timeline.push("_");
  ...
  steps.push({ title: `âœ“ All Tasks Done in ${time} Units`, detail: `Timeline: [${timeline.join(", ")}]. Idle: ${timeline.filter(t=>t==="_").length}. Greedy max-freq minimized total.`, heap: snapH(), phase: "done", codeHL: [16], highlighted: [], extra: { timeline: [...timeline], cooldownQ: snapQ(), expected, result: time } });
  return steps;
}
...
function IOPanel({ step, pKey }) {
  ...
  if (pKey === "kthLargest") return (
    ...
    <div className="border-t border-zinc-800 pt-3"><div className="flex items-center gap-2 mb-1.5"><div className="text-[10px] font-bold uppercase tracking-widest mb-1.5 text-emerald-400">Output (building)</div>{done && extra.result===extra.expected && <span className="text-[9px] bg-emerald-900 text-emerald-300 px-1.5 py-0.5 rounded font-bold">âœ“ MATCH</span>}</div><div className="font-mono text-xs"><span className="text-zinc-500">heap = </span><span className="text-zinc-300">[{step.heap.join(", ")}]</span></div>{done && <div className="font-mono text-xs mt-1"><span className="text-zinc-500">answer = </span><span className="text-emerald-300 font-bold">{extra.result}</span></div>}</div></div>
  );
  ...
  if (pKey === "mergeKLists") return (
    ...
    <div className="border-t border-zinc-800 pt-3"><div className="flex items-center gap-2 mb-1.5"><div className="text-[10px] font-bold uppercase tracking-widest mb-1.5 text-emerald-400">Output (building)</div>{done && <span className="text-[9px] bg-emerald-900 text-emerald-300 px-1.5 py-0.5 rounded font-bold">âœ“ MATCH</span>}</div><div className="font-mono text-[10px] flex flex-wrap gap-0.5"><span className="text-zinc-500">[</span>{DATA_MERGE.expected.map((v,i)=>{const built=(i < extra.merged.length);return <span key={i}><span className={built?"text-emerald-300 font-bold":"text-zinc-700"}>{built?extra.merged[i]:"?"}</span>{(i < DATA_MERGE.expected.length-1)&&<span className="text-zinc-700">,</span>}</span>;})}<span className="text-zinc-500">]</span></div></div></div>
  );

  if (pKey === "kClosest") return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 space-y-3"><div><div className="text-[10px] font-bold uppercase tracking-widest mb-1.5 text-teal-400">Input</div><div className="font-mono text-[10px] text-zinc-400" style={{whiteSpace:"pre"}}>{DATA_CLOSEST.points.map((p,i)=><div key={i}><span className="text-zinc-300">[{p[0]},{p[1]}]</span><span className="text-zinc-600"> dÂ²={DISTS[i]}</span></div>)}<div><span className="text-zinc-500">k</span> = <span className="text-blue-400">{DATA_CLOSEST.k}</span></div></div></div>
    ...
    <div className="border-t border-zinc-800 pt-3"><div className="flex items-center gap-2 mb-1.5"><div className="text-[10px] font-bold uppercase tracking-widest mb-1.5 text-emerald-400">Output (building)</div>{done && <span className="text-[9px] bg-emerald-900 text-emerald-300 px-1.5 py-0.5 rounded font-bold">âœ“ MATCH</span>}</div><div className="font-mono text-xs">{done?<span className="text-emerald-300 font-bold">[{extra.result.map(p=>`[${p}]`).join(", ")}]</span>:<span className="text-zinc-500">heap: {step.heap.length}/{DATA_CLOSEST.k}</span>}</div></div></div>
  );
  ...
  if (pKey === "median") return (
    ...
    <div className="border-t border-zinc-800 pt-3"><div className="flex items-center gap-2 mb-1.5"><div className="text-[10px] font-bold uppercase tracking-widest mb-1.5 text-emerald-400">Output (building)</div>{done && <span className="text-[9px] bg-emerald-900 text-emerald-300 px-1.5 py-0.5 rounded font-bold">âœ“ MATCH</span>}</div><div className="font-mono text-[10px] flex flex-wrap gap-0.5"><span className="text-zinc-500">[</span>{DATA_MEDIAN.expected.map((v,i)=>{const built=(i < extra.medians.length);const match=built&&extra.medians[i]===v;return <span key={i}><span className={match?"text-emerald-300 font-bold":built?"text-zinc-300":"text-zinc-700"}>{built?fmt(extra.medians[i]):"?"}</span>{(i < DATA_MEDIAN.expected.length-1)&&<span className="text-zinc-700">,</span>}</span>;})}<span className="text-zinc-500">]</span></div></div></div>
  );
  ...
  return (
    ...
    <div className="border-t border-zinc-800 pt-3"><div className="flex items-center gap-2 mb-1.5"><div className="text-[10px] font-bold uppercase tracking-widest mb-1.5 text-emerald-400">Output (building)</div>{done&&extra.result===DATA_SCHED.expected&&<span className="text-[9px] bg-emerald-900 text-emerald-300 px-1.5 py-0.5 rounded font-bold">âœ“ MATCH</span>}</div><div className="font-mono text-xs"><span className="text-zinc-500">time = </span><span className={done?"text-emerald-300 font-bold":"text-zinc-300"}>{extra.timeline.length||"?"}</span></div>{extra.timeline.length>0&&<div className="flex gap-1 flex-wrap mt-1">{extra.timeline.map((t,i)=><span key={i} className={`px-1 py-0.5 rounded text-[9px] font-mono font-bold ${t==="_"?"bg-zinc-800 text-zinc-600":"bg-purple-950 border border-purple-800 text-purple-300"}`}>{t}</span>)}</div>}</div></div>
  );
}
...
function HeapVizPanel({ step, pKey }) {
  ...
  if (pKey === "median") {
    ...
    return (
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3 space-y-2">
        <div className="text-[10px] text-zinc-500 mb-0.5">dual-heap â€¢ |lo|={lo.length}, |hi|={hi.length}</div>
        <div><div className="text-[9px] font-bold text-amber-400 uppercase tracking-widest mb-0.5 text-center">maxHeap (lower) {lo.length?`Â· top=${lo[0]}`:""}</div><HeapTreeView values={lo} highlighted={phase==="done"?[0]:[]} phase={phase}/></div>
        <div className="border-t border-zinc-800 pt-1"><div className="text-[9px] font-bold text-blue-400 uppercase tracking-widest mb-0.5 text-center">minHeap (upper) {hi.length?`Â· top=${hi[0]}`:""}</div><HeapTreeView values={hi} highlighted={phase==="done"?[0]:[]} phase={phase}/></div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3">
      <div className="text-[10px] text-zinc-500 mb-1">{type}-heap â€¢ {values.length} node(s)</div>
...
function NavBar({ si, setSi, total }) {
  return (
    <div className="flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded-2xl px-5 py-3">
      ...
      <button onClick={()=>setSi(Math.min(total-1,si+1))} disabled={si>=total-1} className="px-5 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-25 text-sm font-medium rounded-xl transition-colors">Next â†’</button>
    </div>
  );
}
...
function MiddlePanel({ step, pKey, si, totalSteps }) {
  ...
  return (
    <div className="col-span-5 space-y-3">
      ...
      {pKey==="kthLargest" && <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3"><div className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2">Input Array</div><div className="flex gap-1.5 justify-center">{DATA_KTH.arr.map((v,i)=>{const cur=extra.inputIdx===i;const dn=extra.inputIdx>i;return <div key={i} className="flex flex-col items-center gap-1"><span className="text-[9px] text-zinc-600 font-mono">{i}</span><div className={`w-10 text-center py-1 rounded-lg font-mono text-xs font-bold border transition-all ${cur?"bg-blue-950 border-blue-600 text-blue-200 scale-110":dn?"bg-zinc-800 border-zinc-700 text-zinc-500":"bg-zinc-900 border-zinc-700 text-zinc-300"}`}>{v}</div>{cur&&<span className="text-[8px] text-blue-500">â–²</span>}{dn&&<span className="text-[8px] text-zinc-700">âœ“</span>}</div>;})}</div></div>}
      ...
      {pKey==="mergeKLists" && <>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3"><div className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2">Merged â€” {extra.merged.length}/{DATA_MERGE.expected.length}</div><div className="flex gap-1 flex-wrap">{extra.merged.map((v,i)=><span key={i} className="inline-flex items-center justify-center w-7 h-6 rounded bg-emerald-950 border border-emerald-800 text-emerald-300 text-[10px] font-mono font-bold">{v}</span>)}{extra.merged.length===0&&<span className="text-[10px] text-zinc-600 italic">empty</span>}</div></div>
      </>}

      {/* kClosest: points tracker */}
      {pKey==="kClosest" && <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3"><div className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2">Points</div><div className="flex gap-1.5 flex-wrap justify-center">{DATA_CLOSEST.points.map((p,i)=>{const cur=extra.inputIdx===i;const dn=extra.inputIdx>i;return <div key={i} className="flex flex-col items-center gap-1"><span className="text-[9px] text-zinc-600 font-mono">{i}</span><div className={`px-1.5 text-center py-1 rounded-lg font-mono text-[10px] font-bold border transition-all ${cur?"bg-blue-950 border-blue-600 text-blue-200 scale-110":dn?"bg-zinc-800 border-zinc-700 text-zinc-500":"bg-zinc-900 border-zinc-700 text-zinc-300"}`}>({p[0]},{p[1]})</div>{cur&&<span className="text-[8px] text-blue-500">â–²</span>}{dn&&<span className="text-[8px] text-zinc-700">âœ“</span>}</div>;})}</div></div>}
      ...
      {pKey==="median" && <>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3"><div className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2">Heap Sizes & Median</div><div className="flex gap-4"><div className="flex-1 text-center"><div className="text-xl font-bold font-mono text-amber-400">{extra.lo.length}</div><div className="text-[9px] text-zinc-600">maxHeap</div></div><div className="flex-1 text-center"><div className="text-xl font-bold font-mono text-blue-400">{extra.hi.length}</div><div className="text-[9px] text-zinc-600">minHeap</div></div><div className="flex-1 text-center"><div className="text-xl font-bold font-mono text-emerald-400">{extra.medians.length>0?(extra.medians[extra.medians.length-1]%1===0?extra.medians[extra.medians.length-1]:extra.medians[extra.medians.length-1].toFixed(1)):"â€”"}</div><div className="text-[9px] text-zinc-600">median</div></div></div></div>
      </>}

      {/* taskScheduler: timeline + cooldown + stats */}
      {pKey==="taskScheduler" && <>
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3"><div className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2">Timeline â€” t={extra.timeline.length}</div><div className="flex gap-1 flex-wrap">{extra.timeline.map((t,i)=><div key={i} className="flex flex-col items-center"><span className="text-[8px] text-zinc-700 font-mono">{i+1}</span><span className={`w-7 h-7 rounded flex items-center justify-center text-[10px] font-mono font-bold ${t==="_"?"bg-zinc-800 text-zinc-600 border border-zinc-700":"bg-purple-950 border border-purple-700 text-purple-300"}`}>{t}</span></div>)}{extra.timeline.length===0&&<span className="text-[10px] text-zinc-600 italic">empty</span>}</div></div>
        {extra.cooldownQ.length>0 && <div className="bg-amber-950/20 border border-amber-900/50 rounded-2xl p-3"><div className="text-[10px] font-semibold text-amber-400 uppercase tracking-wider mb-1.5">Cooldown Queue</div><div className="flex gap-2 flex-wrap">{extra.cooldownQ.map((q,i)=><span key={i} className="px-2 py-1 bg-amber-950 border border-amber-800 rounded-lg text-amber-300 font-mono text-[10px]">{q[1]}(Ã—{q[0]}) @t={q[2]}</span>)}</div></div>}
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-3 sm:p-4" style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif" }}>
      <div className="max-w-7xl mx-auto">
        <div className="mb-3 flex items-end justify-between flex-wrap gap-2">
          <div><h1 className="text-2xl font-bold tracking-tight">Heap & Priority Queue</h1><p className="text-zinc-500 text-sm mt-0.5">O(log n) Insert & Extract Â· The Engine Behind Greedy Selection</p></div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4"><div className="text-[10px] font-semibold text-blue-400 uppercase tracking-wider mb-2">When to Use</div><ul className="space-y-1.5 text-xs text-zinc-400"><li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">â€º</span>"Find kth largest/smallest" or "top-k" â€” min-heap of size k</li><li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">â€º</span>Repeated "get max/min" from a changing collection</li><li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">â€º</span>Merge k sorted streams â€” min-heap of k pointers</li><li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">â€º</span>Running median / order statistics â€” two heaps</li><li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">â€º</span>Greedy scheduling with cooldowns â€” max-heap + queue</li></ul><div className="mt-3 pt-3 border-t border-zinc-800"><div className="text-[10px] text-zinc-600 space-y-1"><div><span className="text-zinc-500 font-semibold">Push:</span> O(log n)</div><div><span className="text-zinc-500 font-semibold">Pop:</span> O(log n)</div><div><span className="text-zinc-500 font-semibold">Peek:</span> O(1)</div><div><span className="text-zinc-500 font-semibold">Heapify:</span> O(n) â€” not O(n log n)</div><div><span className="text-zinc-500 font-semibold">Python:</span> heapq is min-heap; negate for max</div></div></div></div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4"><div className="text-[10px] font-semibold text-amber-400 uppercase tracking-wider mb-2">Classic Problems</div><div className="space-y-1.5 text-xs"><div className="flex items-center gap-2"><span className="text-amber-500/60">â€¢</span><span className="text-zinc-400">LC 215 â€” Kth Largest Element</span><span className="ml-auto text-[10px] text-amber-700">Medium</span></div><div className="flex items-center gap-2"><span className="text-amber-500/60">â€¢</span><span className="text-zinc-400">LC 23 â€” Merge K Sorted Lists</span><span className="ml-auto text-[10px] text-red-700">Hard</span></div><div className="flex items-center gap-2"><span className="text-amber-500/60">â€¢</span><span className="text-zinc-400">LC 973 â€” K Closest Points to Origin</span><span className="ml-auto text-[10px] text-amber-700">Medium</span></div><div className="flex items-center gap-2"><span className="text-amber-500/60">â€¢</span><span className="text-zinc-400">LC 295 â€” Find Median from Data Stream</span><span className="ml-auto text-[10px] text-red-700">Hard</span></div><div className="flex items-center gap-2"><span className="text-amber-500/60">â€¢</span><span className="text-zinc-400">LC 621 â€” Task Scheduler</span><span className="ml-auto text-[10px] text-amber-700">Medium</span></div><div className="flex items-center gap-2"><span className="text-amber-500/60">â€¢</span><span className="text-zinc-400">LC 347 â€” Top K Frequent Elements</span><span className="ml-auto text-[10px] text-amber-700">Medium</span></div><div className="flex items-center gap-2"><span className="text-amber-500/60">â€¢</span><span className="text-zinc-400">LC 1046 â€” Last Stone Weight</span><span className="ml-auto text-[10px] text-amber-700">Easy</span></div></div></div>
        </div>
      </div>
    </div>
  );
