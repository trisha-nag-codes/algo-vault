...
/* â”€â”€â”€ Adjacency list for IO display â”€â”€â”€ */
...
function buildSteps() {
  ...
  steps.push({
    title: "Initialize â€“ Source Distance = 0",
    detail: `dist[${SOURCE}] = 0, all others = âˆž. Run Vâˆ’1 = ${N - 1} rounds of edge relaxation. Each round processes ALL ${EDGES.length} edges.`,
  ...
  for (let round = 1; round <= N - 1; round++) {
    let roundChanges = 0;
    steps.push({
      title: `Round ${round} of ${N - 1} â€“ Relax All Edges`,
    ...
    for (let e = 0; e < EDGES.length; e++) {
      ...
      if (improved) {
        dist[v] = dist[u] + w;
        roundChanges++;
        relaxedEdges.push({ round, edge: `${u}â†’${v}`, from: prevDist[v], to: dist[v] });
      }

      steps.push({
        title: improved
          ? `Edge ${u}â†’${v} (w=${w}): Relax ${prevDist[v] === Infinity ? "âˆž" : prevDist[v]} â†’ ${dist[v]}`
          : `Edge ${u}â†’${v} (w=${w}): No Improvement`,
        detail: improved
          ? `dist[${u}] + w = ${prevDist[u]} + ${w < 0 ? `(${w})` : w} = ${dist[v]} < ${prevDist[v] === Infinity ? "âˆž" : prevDist[v]}. Update dist[${v}].`
          : dist[u] === Infinity
            ? `dist[${u}] = âˆž. Cannot relax â€” source not yet reachable.`
            : `dist[${u}] + w = ${dist[u]} + ${w < 0 ? `(${w})` : w} = ${dist[u] + w} â‰¥ ${dist[v]}. No improvement.`,
    ...
    if (roundChanges === 0) {
      steps.push({
        title: `Round ${round} â€“ No Changes, Early Termination`,
  ...
  steps.push({
    title: "âœ“ Complete â€“ Shortest Distances Found",
    detail: `Final distances from node ${SOURCE}: [${dist.join(", ")}]. The negative edge 2â†’1 (w=âˆ’1) gave a shorter path: 0â†’2â†’1 (cost 1) vs 0â†’1 (cost 6).`,
      ...
      {POS.map((pos, id) => {
        ...
        return (
          <g key={id}>
            ...
            <text x={pos.x} y={pos.y - 30} textAnchor="middle" fill={d === Infinity ? "#52525b" : "#a1a1aa"} fontSize="11" fontFamily="monospace">{d === Infinity ? "âˆž" : d}</text>
          </g>
        );
      })}
    </svg>
          ...
          <div className="mt-2 space-y-0.5">
            {relaxedEdges.slice(-6).map((r, i) => (
              <div key={i} className="flex items-center gap-1.5 text-[10px]">
                ...
                <span className="text-zinc-700">{r.from === Infinity ? "âˆž" : r.from}â†’{r.to}</span>
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-3 sm:p-4" style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif" }}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-3">
          ...
          <p className="text-zinc-500 text-sm mt-0.5">Shortest Path with Negative Edges â€¢ Vâˆ’1 Relaxation Rounds</p>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl px-5 py-3 mb-3">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed mt-1">
            Bellman-Ford relaxes <strong className="text-zinc-300">every edge</strong> in <strong className="text-zinc-300">Vâˆ’1 rounds</strong>. Unlike Dijkstra, it handles <strong className="text-amber-400">negative edge weights</strong> because it doesn't greedily finalize distances â€” each round propagates improvements one hop further. A V-th round detecting further improvement means a <strong className="text-zinc-300">negative cycle</strong>. Edge 2â†’1 (w=<span className="text-amber-400 font-mono">âˆ’1</span>) is dashed to highlight the negative weight.
          </p>
        </div>
        ...
        <div className="grid grid-cols-12 gap-3">
          ...
          <div className="col-span-3 space-y-3">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3">
              <div className="text-[10px] text-zinc-500 mb-1">Source: node {SOURCE} â€¢ {N}N, {EDGES.length}E â€¢ dashed = negative</div>
              <GraphView step={step} />
                ...
                {EDGES.map(([u, v, w], i) => {
                  ...
                  return (
                    <div key={i} className={`flex items-center gap-2 px-2 py-0.5 rounded-lg font-mono text-xs ${
                      ...
                      <span className={isActive ? "text-blue-400" : "text-zinc-500"}>{u} â†’ {v}</span>
                      <span className={`px-1 rounded ${w < 0 ? "text-amber-400" : "text-zinc-500"}`}>w={w}</span>
                      {isActive && <span className="text-blue-500 text-[10px] ml-auto">â—€ current</span>}
                    </div>
                  );
                })}
              </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
            ...
            <ul className="space-y-1.5 text-xs text-zinc-400">
              <li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">â€º</span>Shortest path with negative edge weights</li>
              <li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">â€º</span>Detecting negative cycles (run V-th round)</li>
              <li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">â€º</span>When Dijkstra isn't applicable (negative weights present)</li>
              <li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">â€º</span>Arbitrage detection in currency exchange graphs</li>
            </ul>
            <div className="mt-3 pt-3 border-t border-zinc-800">
              <div className="text-[10px] text-zinc-600 space-y-1">
                <div><span className="text-zinc-500 font-semibold">Time:</span> O(V Ã— E)</div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
            ...
            <div className="space-y-1.5 text-xs">
              ...
              <div className="flex items-center gap-2"><span className="text-amber-500/60">â€¢</span><span className="text-zinc-400">LC 1334 â€” Find the City With Smallest Neighbors</span><span className="ml-auto text-[10px] text-amber-700">Medium</span></div>
              <div className="flex items-center gap-2"><span className="text-amber-500/60">â€¢</span><span className="text-zinc-400">LC 1514 â€” Path with Maximum Probability</span><span className="ml-auto text-[10px] text-amber-700">Medium</span></div>
              ...
              <div className="flex items-center gap-2"><span className="text-amber-500/60">â€¢</span><span className="text-zinc-400">LC 2307 â€” Check for Contradictions in Equations</span><span className="ml-auto text-[10px] text-red-700">Hard</span></div>
            </div>
          </div>
        </div>
      </div>
